 ...
 */
void run() {
    if (time_quantum <= 0 || time_tick <= 0) { // Check if any of those values are invalid.
        printf("[-] Time quantum and time tick must be greater than 0.\n");
        return;
    }

    printf("[+] Starting job...\n");
    printf("    Time Quantum : %d time ticks\n", time_quantum);
